...
# Batch Prediction 
    ...
    file = st.file_uploader("Upload CSV", type="csv", key="batch_upload")
    if file:
        try:
            # Try reading with ISO-8859-1 first, fallback to utf-8
            try:
            ...
            if 'OriginalTweet' not in df.columns:
                ...
                fig, ax = plt.subplots()
                df['PredictedSentiment'].value_counts().plot.pie(
                    autopct='%1.1f%%', startangle=90, shadow=True, ax=ax
                )
                ax.set_ylabel("")
                st.pyplot(fig)
